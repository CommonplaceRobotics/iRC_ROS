...
MessageType CriMessage::GetMessageType(const std::string & msg)
{
  ...
  static const std::map<std::string, MessageType> message_type_map = {
    ...
    {cri_keywords::TYPE_CMDERROR, MessageType::CMDERROR},
    //{cri_keywords::TYPE_CYCLESTAT, MessageType::CYCLESTAT}
  };
...
Status::Status(const std::string & messageString, int criVersion) : CriMessage(MessageType::STATUS)
{
  ...
  case 16:
      ...
      digital_in = std::stol(dinString, nullptr, 2);    // TODO: Process further to actual meaning
      digital_out = std::stol(doutString, nullptr, 2);  // TODO: Process further to actual meaning
      break;
  case 17:
      // FIXME (MAB): DIO have 64 bit but only 32 bit are read here!
      digital_in = std::stol(dinString, nullptr, 16);    // TODO: Process further to actual meaning
      digital_out = std::stol(doutString, nullptr, 16);  // TODO: Process further to actual meaning
      break;
    default:
